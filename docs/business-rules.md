...
| Confirm upload | Client-asserted signal that the document bytes have been fully uploaded to storage. This is the trigger that moves the request out of `PENDING_UPLOAD` and enqueues processing. The backend does not otherwise learn that an upload happened (see `docs/design-trade-offs.md § Document upload flow`). |
...
- Requests failing structural validation are rejected synchronously with a `400`-class Problem Details response (see `docs/design-trade-offs.md § API error format`) — they never reach `PENDING_UPLOAD`.
...
Rationale for the specific thresholds (PDF-only, 15 MB ceiling) is recorded in `docs/design-trade-offs.md § Deterministic verdict rule for the processing stub`.
